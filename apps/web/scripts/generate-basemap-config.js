...
const DARK_STYLE_BINDING_KEY = "weltgewebe:darkStyleSha256";

function verifyDarkStyleBinding(lightPath, darkPath) {
  let lightStyle;
  try {
    lightStyle = JSON.parse(fs.readFileSync(lightPath, "utf8"));
  } catch (error) {
    throw new Error(`Invalid basemap style JSON: ${lightPath}`, {
      cause: error,
    });
  }
  const darkSha256 = sha256File(darkPath);
  const boundSha256 = lightStyle?.metadata?.[DARK_STYLE_BINDING_KEY];
  if (boundSha256 !== darkSha256) {
    throw new Error(
      `Basemap dark-style binding mismatch: ${lightPath} must bind ${darkPath} as ${darkSha256}`,
    );
  }
}
...
const buildIdentity =
  mode === "local-sovereign"
    ? (() => {
        ...
        verifyDarkStyleBinding(lightPath, darkPath);
        return {
          schema_version: 1,
          mode,
          variant,
          ...
          // The light style embeds the exact dark-style SHA-256, so this
          // established identity transitively binds both runtime style files.
