...
test.describe("weltweberei information surface", () => {
  test("is served, self-contained and accessible", async ({
    ...
    await test.step("active-content and origin isolation", async () => {
      // 8. No active or embedded elements, no mailto/tel/javascript links.
      ...
      await expect(page.locator('a[href^="tel:" i]')).toHaveCount(0);
      ...
      // 9./10. Every request stays on the local preview origin (no foreign
      // origin, no CDN). data: URLs are inert and therefore tolerated.
      for (const req of requests) {
        const url = req.url();
        if (url.startsWith("data:")) continue;
        expect(
          new URL(url).origin,
          `request to unexpected origin: ${url}`,
        ).toBe(pageOrigin);
      }
    });

    await test.step("stylesheet delivery contract", async () => {
      // The stylesheet must be present exactly once, requested locally, and
      // must be answered successfully with a CSS content type — a 404 fails.
      ...
      expect(
        stylesheetResponses,
        "exactly one stylesheet response",
      ).toHaveLength(1);
    ...
    await test.step("pre-activation metadata contract", async () => {
      // Not indexable, no canonical link to the target domain until the
      // activation gate is cleared.
    ...
    await test.step("contact-data guard", async () => {
      // 14. No known legacy private/contact data leaked into the artifact.
      // Comparison is case-insensitive so e.g. "MailTo:" cannot slip through.
      ...
      for (const needle of FORBIDDEN_CONTACT) {
        expect(
          normalizedHtml,
          `artifact must not contain "${needle}"`,
    ...
    await test.step("narrow-layout contract", async () => {
      ...
      expect(overflow, "no horizontal overflow at 320px").toBeLessThanOrEqual(
        0,
      );
    });

    await test.step("keyboard and focus contract", async () => {
