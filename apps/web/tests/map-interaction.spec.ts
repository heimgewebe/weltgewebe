...
test.describe("Map Interaction & Context Panel", () => {
  ...
  test("NodePanel keyboard navigation allows arrow keys, Home, and End", async ({ page }) => {
    await page.waitForSelector(".map-marker", { timeout: 10000 });
    ...
    await page.evaluate(() => {
      ...
      const nodeMarker = markers.find(m => !m.classList.contains('garnrolle-marker')) || markers[0];
    ...
    const uebersichtTab = panel.locator('button[role="tab"]', { hasText: 'Übersicht' });
    ...
    const gespraechTab = panel.locator('button[role="tab"]', { hasText: 'Gespräch' });
    ...
    const verlaufTab = panel.locator('button[role="tab"]', { hasText: 'Verlauf' });
  ...
  test("AccountPanel keyboard navigation allows arrow keys, Home, and End", async ({ page }) => {
    await page.waitForSelector(".map-marker", { timeout: 10000 });

    // Open an account. In our mock data, accounts usually use garnrolle markers.
    await page.evaluate(() => {
      const markers = Array.from(document.querySelectorAll('.map-marker')) as HTMLElement[];
      const accountMarker = markers.find(m => m.classList.contains('garnrolle-marker')) || markers[markers.length - 1];
    ...
    const profilTab = panel.locator('button[role="tab"]', { hasText: 'Profil' });
    ...
    await page.keyboard.press("ArrowRight");
    const aktivitaetTab = panel.locator('button[role="tab"]', { hasText: 'Aktivität' });
    ...
    await page.keyboard.press("End");
    const knotenTab = panel.locator('button[role="tab"]', { hasText: 'Knoten' });
