...
test.describe("uiView store state transitions", () => {
  ...
  test("enterKomposition sets selection === null and kompositionDraft !== null", () => {
    ...
    expect(get(kompositionDraft)).toEqual({ mode: "new-knoten", source: "action-bar" });
    expect(get(contextPanelOpen)).toBe(true);
  });
  ...
  test("assertUiStateInvariant throws on invalid states", () => {
    // Both set
    expect(() => assertUiStateInvariant("navigation", { type: "node", id: "1" }, { mode: "new-knoten", source: "action-bar" }))
      .toThrow(/cannot both be set at the same time/);

    // fokus but no selection
    expect(() => assertUiStateInvariant("fokus", null, null))
      .toThrow(/systemState is 'fokus' but selection is null/);

    // navigation but selection set
    expect(() => assertUiStateInvariant("navigation", { type: "node", id: "1" }, null))
      .toThrow(/systemState is 'navigation' but selection is not null/);

    // komposition but no draft
    expect(() => assertUiStateInvariant("komposition", null, null))
      .toThrow(/systemState is 'komposition' but kompositionDraft is null/);

    // not komposition but draft set
    expect(() => assertUiStateInvariant("navigation", null, { mode: "new-knoten", source: "action-bar" }))
      .toThrow(/systemState is not 'komposition' but kompositionDraft is not null/);
  });
});
