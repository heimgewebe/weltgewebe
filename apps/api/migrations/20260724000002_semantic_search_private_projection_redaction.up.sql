...
-- Private and unknown-visibility nodes remain represented for completeness,
-- but only as content-free, non-semantic sentinels. Public rows keep the exact
-- embedding and authorization contract.
...
SELECT COALESCE((
    SELECT g.state IN ('building', 'ready', 'active')
       AND g.document_revision = 'node-document-v2-public-only'
             ...
             AND NOT EXISTS (
                 ...
                 WHERE p.generation_id = g.generation_id
                   ...
                   AND (
                       ...
                       OR
                       (coalesce(n.payload ->> 'search_visibility', '') <> 'public'
                        AND p.status = 'hidden'
