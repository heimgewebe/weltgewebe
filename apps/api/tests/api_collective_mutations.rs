...
#[tokio::test]
#[serial]
async fn weber_can_replace_shared_node_update_edge_and_delete_node_cascade() -> Result<()> {
    ...
    let patch_edge = mutation_request(
        "PATCH",
        "/edges/e1",
        &cookie,
        r#"{"edge_kind":"membership"}"#,
    );
    let response = app.clone().oneshot(patch_edge).await?;
    assert_eq!(response.status(), StatusCode::OK);
    let bytes = body::to_bytes(response.into_body(), usize::MAX).await?;
    let edge: serde_json::Value = serde_json::from_slice(&bytes)?;
    assert_eq!(edge["edge_kind"], "membership");
...
async fn guest_cannot_mutate_shared_elements() -> Result<()> {
    ...
    let delete_edge = mutation_request("DELETE", "/edges/e1", &cookie, "");
    assert_eq!(
        app.oneshot(delete_edge).await?.status(),
        StatusCode::FORBIDDEN
    );
