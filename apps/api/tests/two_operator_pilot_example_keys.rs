...
const EXAMPLE: &str =
    include_str!("../../../platform/cell-pilot/two-operator-pilot.example.invalid.json");
...
fn public_example_keys_are_runtime_valid_digest_bound_and_reciprocal() {
    ...
    for (index, cell) in cells.iter().enumerate() {
        let active = cell["identity"]["active_public_key"]
            .as_str()
            .expect("active public key must be a string");
        ...
        let digest = Sha256::digest(bytes)
            ...
            .collect::<String>();
        assert_eq!(
            cell["identity"]["public_key_sha256"].as_str(),
            Some(digest.as_str())
        );
        assert_eq!(
            cell["peer"]["expected_public_key"].as_str(),
            Some(RFC8032_PUBLIC_KEYS[1 - index])
        );
    }
}
