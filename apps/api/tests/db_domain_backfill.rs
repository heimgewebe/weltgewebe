...
async fn import_nodes(pool: &sqlx::PgPool, content: &str) -> BackfillReport {
    ...
    for line in content.lines() {
        ...
        let id = match v.get("id").and_then(|v| v.as_str()).filter(|s| !s.is_empty()) {
            Some(s) => s.to_string(),
            None => {
        ...
        let title = v
            ...
            .unwrap_or("Untitled")
            .to_string();
        let lat = v.get("location").and_then(|l| l.get("lat")).and_then(f64_from_value);
        let lon = v.get("location").and_then(|l| l.get("lon")).and_then(f64_from_value);
...
async fn domain_backfill_nodes_deterministic_and_idempotent() {
    ...
    assert_eq!(r1.records_inserted, 3, "first import: all 3 must be new inserts");
    ...
    assert_eq!(r2.records_inserted, 0, "second import: no new rows expected");
    assert_eq!(r2.records_updated, 3, "second import: all 3 must be updates");
    ...
    assert_eq!(count, 3, "idempotent re-import must not create duplicate rows");
...
async fn domain_backfill_edges_deterministic_and_idempotent() {
    ...
    let (source_id, target_id, edge_kind): (String, String, String) = sqlx::query_as(
        "SELECT source_id, target_id, edge_kind FROM domain_edges WHERE id = $1",
    )
    ...
    let (count,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM domain_edges WHERE id LIKE 'backfill-proof-edge-%'")
            ...
            .await
            .expect("count query failed");
    assert_eq!(count, 2, "idempotent re-import must not create duplicate rows");
...
async fn domain_backfill_accounts_deterministic_and_idempotent() {
    ...
    assert_eq!(r1.malformed_json_lines, 0);
    assert_eq!(r1.skipped_records, 0);
    assert!(r1.duplicate_emails.is_empty(), "no duplicate emails in clean fixture");
    ...
    assert!((loc_lat.unwrap() - 53.5).abs() < 1e-6, "location_lat must map correctly");
    assert!((loc_lon.unwrap() - 10.0).abs() < 1e-6, "location_lon must map correctly");
    ...
    assert!(beta_lat.is_none(), "ron account must have NULL location_lat");
    assert!(beta_lon.is_none(), "ron account must have NULL location_lon");

    // radius_m type: stored as BIGINT, bound as i64
    let (radius_m,): (i64,) =
        sqlx::query_as("SELECT radius_m FROM domain_accounts WHERE id = $1")
            .bind("backfill-proof-account-alpha")
            .fetch_one(&pool)
            .await
...
async fn domain_backfill_malformed_lines_quarantined() {
    ...
    assert_eq!(r.malformed_json_lines, 1, "exactly one malformed line must be quarantined");
...
async fn domain_backfill_duplicate_id_converges() {
    ...
    assert_eq!(r.records_updated, 1, "second occurrence must be an update (ON CONFLICT)");
    ...
    assert_eq!(kind, "Second", "last-write-wins: second record's kind must prevail");
...
async fn domain_backfill_duplicate_account_emails_audited() {
    ...
    .await
    .expect("count query failed");
    assert_eq!(count, 2, "Phase B allows both duplicate-email accounts to be imported");
