//! Integration proof: SQLx → PgBouncer (transaction mode) → PostgreSQL → sessions CRUD.
//!
//! This test is `#[ignore]` and requires an external stack.
//! Run with:
//!   PGBOUNCER_URL=postgres://welt:gewebe@localhost:6432/weltgewebe \
//!     cargo test -p weltgewebe-api -- sqlx_pgbouncer --include-ignored
//!
//! What is proven:
//! - sqlx::PgPool connects through PgBouncer in transaction pool mode.
//! - PgConnectOptions::statement_cache_capacity(0) neutralises the prepared-statement
//!   leak that would otherwise occur under PgBouncer transaction mode.
//! - INSERT / SELECT / UPDATE / DELETE on the sessions table all succeed via SQLx,
//!   not via psql.
//!
//! What is NOT proven here:
//! - sqlx::migrate! / sqlx-cli migration run (separate proof step).
//! - Production SessionStore wiring (no DbSessionStore in this PR).

use chrono::{Duration, Utc};
use sqlx::postgres::{PgConnectOptions, PgPoolOptions};
use sqlx::Row;
use std::str::FromStr;
use uuid::Uuid;

/// Idempotent setup: create the sessions table if it does not already exist.
/// Uses the exact schema from apps/api/migrations/20260428000000_create_sessions.up.sql.
async fn ensure_sessions_table(pool: &sqlx::PgPool) {
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS sessions (\
            id          TEXT        PRIMARY KEY,\
            account_id  TEXT        NOT NULL,\
            device_id   TEXT        NOT NULL,\
            created_at  TIMESTAMPTZ NOT NULL,\
            last_active TIMESTAMPTZ NOT NULL,\
            expires_at  TIMESTAMPTZ NOT NULL\
        )",
    )
    .execute(pool)
    .await
    .expect("failed to ensure sessions table exists");
}

/// Proof: SQLx CRUD against the sessions table through PgBouncer transaction mode.
///
/// Covers: connection, INSERT, SELECT (round-trip), UPDATE (touch last_active), DELETE,
/// post-delete COUNT verification.
///
/// statement_cache_capacity(0) is the documented mitigation for the SQLx prepared-statement
/// cache incompatibility with PgBouncer transaction mode. Without it, PgBouncer may return
/// "prepared statement does not exist" errors when a connection is reused from the pool
/// and a cached statement handle references a statement prepared on a different backend
/// connection.
#[tokio::test]
#[ignore = "requires PGBOUNCER_URL and a running PostgreSQL+PgBouncer stack in transaction mode"]
async fn sqlx_pgbouncer_session_crud_through_transaction_mode() {
    let pgbouncer_url = match std::env::var("PGBOUNCER_URL") {
        Ok(url) => url,
        Err(_) => {
            eprintln!("PGBOUNCER_URL not set — skipping SQLx/PgBouncer proof test");
            return;
        }
    };

    let connect_opts = PgConnectOptions::from_str(&pgbouncer_url)
        .expect("PGBOUNCER_URL must be a valid postgres connection string")
        .statement_cache_capacity(0);

    let pool = PgPoolOptions::new()
        .max_connections(2)
        .connect_with(connect_opts)
        .await
        .expect("failed to connect to PgBouncer — is the stack running?");

    ensure_sessions_table(&pool).await;

    let id = Uuid::new_v4().to_string();
    let account_id = format!("proof-account-{}", Uuid::new_v4());
    let device_id = format!("proof-device-{}", Uuid::new_v4());
    let now = Utc::now();
    let expires_at = now + Duration::days(1);

    // --- INSERT ---
    let insert_result = sqlx::query(
        "INSERT INTO sessions (id, account_id, device_id, created_at, last_active, expires_at) \
         VALUES ($1, $2, $3, $4, $5, $6)",
    )
    .bind(&id)
    .bind(&account_id)
    .bind(&device_id)
    .bind(now)
    .bind(now)
    .bind(expires_at)
    .execute(&pool)
    .await
    .expect("INSERT into sessions failed");

    assert_eq!(
        insert_result.rows_affected(),
        1,
        "INSERT must affect exactly one row"
    );

    // --- SELECT (round-trip verification) ---
    let row = sqlx::query("SELECT id, account_id, device_id FROM sessions WHERE id = $1")
        .bind(&id)
        .fetch_one(&pool)
        .await
        .expect("SELECT from sessions failed");

    assert_eq!(row.get::<String, _>("id"), id, "id must round-trip");
    assert_eq!(
        row.get::<String, _>("account_id"),
        account_id,
        "account_id must round-trip"
    );
    assert_eq!(
        row.get::<String, _>("device_id"),
        device_id,
        "device_id must round-trip"
    );

    // --- UPDATE last_active (mirrors SessionStore::touch semantics) ---
    let new_last_active = now + Duration::minutes(10);
    let update_result = sqlx::query("UPDATE sessions SET last_active = $1 WHERE id = $2")
        .bind(new_last_active)
        .bind(&id)
        .execute(&pool)
        .await
        .expect("UPDATE on sessions failed");

    assert_eq!(
        update_result.rows_affected(),
        1,
        "UPDATE must affect exactly one row"
    );

    // --- SELECT after UPDATE (verify write persisted) ---
    let after_update_row = sqlx::query("SELECT last_active FROM sessions WHERE id = $1")
        .bind(&id)
        .fetch_one(&pool)
        .await
        .expect("SELECT after UPDATE failed");

    let stored_last_active: chrono::DateTime<Utc> = after_update_row.get("last_active");
    let diff_ms = (stored_last_active - new_last_active)
        .num_milliseconds()
        .abs();
    assert!(
        diff_ms < 1000,
        "last_active after UPDATE must be within 1 s of written value (diff: {diff_ms} ms)"
    );

    // --- DELETE ---
    let delete_result = sqlx::query("DELETE FROM sessions WHERE id = $1")
        .bind(&id)
        .execute(&pool)
        .await
        .expect("DELETE from sessions failed");

    assert_eq!(
        delete_result.rows_affected(),
        1,
        "DELETE must remove exactly one row"
    );

    // --- COUNT after DELETE (confirm gone) ---
    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM sessions WHERE id = $1")
        .bind(&id)
        .fetch_one(&pool)
        .await
        .expect("COUNT after DELETE failed");

    assert_eq!(count, 0, "session must be absent after DELETE");

    pool.close().await;
}
