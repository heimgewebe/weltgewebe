//! Direct disposable-PostgreSQL proofs for T012's generation and migration boundaries.
...
#[tokio::test]
#[ignore = "requires T005_DATABASE_URL pointing to direct disposable PostgreSQL"]
async fn visibility_cutover_does_not_version_or_enqueue_legacy_generations() {
    let pool = pool().await;
    migrate(&pool).await;
    reset(&pool).await;
