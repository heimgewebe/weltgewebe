...
| Passkeys              | optional    | Register-Options mit Grant-Handoff: Step-up erzeugt Grant, `register/options` konsumiert Grant und startet WebAuthn-Ceremony; PasskeyStore + `webauthn_user_id`-Writeback-Mutation vorhanden; `register/verify` API-seitig implementiert (echte `finish_passkey_registration`, single-use, Duplicate-Detection, Writeback) — Negativpfade getestet; positiver Verify-Pfad ist lokal mit echtem Browser-/Authenticator-Flow belegt (`LOCAL_PROOF_ONLY`); verpflichtender CI-Job `auth-passkey-register-proof` in `.github/workflows/auth-passkey-register-proof.yml` hinzugefügt (führt `pnpm test:proof:auth-passkey-register` aus, kein `if:`-Guard) — CI-Job hinzugefügt; grüner Lauf steht noch aus (`READY_FOR_CI_PROOF`); Login-/Management-Pfade offen | Teil  | mittel  |
...
- Lokaler Browser-Proof in `apps/web/tests/proofs/passkey-register-positive.proof.ts` mit `apps/web/playwright.auth.proof.config.ts`: echter `navigator.credentials.create(...)`-Pfad über Chromium-CDP-`WebAuthn.addVirtualAuthenticator`, `POST /auth/passkeys/register/options`, `POST /auth/passkeys/register/verify` → `200 OK`, kein `Set-Cookie`, Session-Cookie unverändert, Credential im `PasskeyStore` sichtbar (`LOCAL_PROOF_ONLY`)
- CI-Job `auth-passkey-register-proof` in `.github/workflows/auth-passkey-register-proof.yml` führt `pnpm test:proof:auth-passkey-register` aus `apps/web` aus (Node/pnpm + Rust/Cargo + Playwright/Chromium); API wird über `playwright.auth.proof.config.ts` mit `cargo run --locked --features integration-testing` gestartet; verbindlich auf `pull_request`/`push` zu `main` (kein `if:`-Guard, kein workflow_dispatch-only). Basemap-Proof bleibt getrennt in `.github/workflows/basemap-runtime-proof.yml`

**CI-Job hinzugefügt; grüner Lauf steht noch aus.** Status bleibt `READY_FOR_CI_PROOF`, bis ein grüner Lauf des Jobs `auth-passkey-register-proof` referenziert werden kann.

**Fehlende Belege:** grüner CI-Lauf des Jobs `auth-passkey-register-proof` für den positiven Browser-Proof (`READY_FOR_CI_PROOF`); Auth-Flow (Options/Verify); persistente Ablage über Neustart; E2E-UI
**Status:** Teil
**Risiko:** mittel
