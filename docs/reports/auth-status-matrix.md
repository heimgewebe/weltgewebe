...
**Code-, Test- und Verifikationsbelege:** `apps/api/src/routes/auth.rs`, `apps/api/src/auth/tokens.rs` (`latest_raw_for_email`), `apps/api/tests/api_auth.rs` (`magic_link_full_round_trip_request_to_session`), `.github/workflows/api.yml` (Job `magic-link-flow-proof`), `apps/web/src/routes/login/+page.svelte`, `verification/verify_magic_link.py`
