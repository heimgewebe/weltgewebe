...
**Ist:** Die MVP-Linie nutzt einen In-Memory Session-Store. `GET /auth/session` wurde als API-Endpoint implementiert (inkl. `expires_at`), lässt `device_id` jedoch derzeit noch aus (omitted). Deckt Persistenzanforderungen, Session Refresh und dynamische `device_id` noch nicht ab.
**Dokumentationsbelege:** `docs/specs/auth-blueprint.md`, `docs/blueprints/weltgewebe.auth-and-ui-routing.md`
**Code-, Test- und Verifikationsbelege:** `apps/api/src/routes/auth.rs`, `apps/api/tests/api_auth.rs`, `apps/api/src/auth/session.rs`, `apps/web/src/lib/auth/store.ts`
