...
lang: de
summary: >
  Diagnosebericht zu OPT-API-002: Ist-Befund aller Auth-Stores, fehlende
  DB-Strukturen, Entscheidung zur Persistenzstrategie und Minimal-Migrationsplan.
...
- PostgreSQL ist die kanonische Persistenzschicht; sie ist in jedem
...
- Neuer CI-Step: `sqlx migrate run` vor `cargo test` in `api.yml`.
