...
| OPT-API-003 | API | DB-Migrationen | partial | code | hoch | niedrig | hoch | `apps/api/migrations/20260428000000_create_sessions.up.sql`, `apps/api/migrations/20260428000000_create_sessions.down.sql`, `apps/api/README.md`, `Justfile` | `cargo test --locked -p weltgewebe-api`; `git diff --check` | Migrationsdateien und Doku vorhanden; Runtime-Migrationsbeweis gegen PostgreSQL, CI-Pflichtpfad und Startup-/Deploy-Strategie fehlen | 2026-04-28 |
