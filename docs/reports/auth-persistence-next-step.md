...
5. Produktive `DbSessionStore`-/`SessionBackend`-Verdrahtung gegen den direkten PostgreSQL-Pfad inkl. Auth-Runtime- und CI-Gate; PgBouncer-Kompatibilitätsprüfung nur für optionale Dev-/Spezialpfade
...
| Direkter Implementierungs-PR wäre zu annahmenreich | **Teilweise wahr** | psql-basierter direkter PostgreSQL-CRUD-Pfad ist belegt; SQLx/Rust-API-CRUD gegen direkten PostgreSQL-Pfad bleibt Runtime-Gate. Es fehlen noch `DbSessionStore`-/`SessionBackend`-Verdrahtung, Auth-Runtime- und CI-Gate sowie async-Abstraktion. PgBouncer ist nach ADR-0007 keine Produktionsvoraussetzung. |
...
| **PostgreSQL** | Stack vorhanden; Migration vorhanden; `db_pool` verdrahtet; keine neue Abhängigkeit; ADR-0007 legt direkten Produktionspfad fest | async-Abstraktion nötig; CI-Gate fehlt; SQLx/Rust-API-CRUD gegen direkten PostgreSQL-Pfad (Runtime-Gate) offen | **empfohlen** (strategisch gestützt; Runtime-Proof offen) |
...
| Verbindlicher Integration-Test-Step mit `DATABASE_URL` | `.github/workflows/api.yml` (neuer **Pflicht**-Job für den DB-backed Session-Pfad; nicht optional) |
...
2. **Direkter SQLx/Postgres-Pfad:** Der direkte `sqlx::PgPool`-CRUD-Proof gegen PostgreSQL ist belegt. Offen für den Folge-PR bleibt die produktive `DbSessionStore`-/`SessionBackend`-Verdrahtung inkl. Auth-Runtime- und CI-Gate. PgBouncer-Varianten (Statement-Cache auf 0, PgBouncer-Upgrade auf ≥ 1.21 mit `max_prepared_statements > 0`, Session-Pooling) sind optionale Dev-/Spezialpfade und keine Produktionsvoraussetzung.
...
| PgBouncer-Kompatibilität? | **optionaler Spezialpfad** | SQLx prepared statements + statement cache + PgBouncer `transaction` mode (`edoburu/pgbouncer:1.20`) sind nicht belastbar kompatibel; nach ADR-0007 aber keine Produktionsvoraussetzung |
| Weiterer vorgelagerter Diagnose-PR nötig? | **nein** | Kein weiterer Diagnose-PR nötig; daraus folgt aber kein `DbSessionStore`-PR. Der psql-basierte direkte PostgreSQL-CRUD-Pfad ist belegt; der SQLx/Rust-API-CRUD gegen direkten PostgreSQL-Pfad bleibt Runtime-Gate. Vor produktivem `DbSessionStore` bleibt zusätzlich die `DbSessionStore`-/`SessionBackend`-Verdrahtung inkl. Auth-Runtime- und CI-Gate erforderlich. PgBouncer bleibt optionaler Dev-/Spezialpfad. |
