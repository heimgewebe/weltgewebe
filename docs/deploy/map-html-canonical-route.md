...
Caddy setzt dafür im VPS-Edge einen eng begrenzten `handle @legacyMapHtml` auf derselben Dispatch-Ebene wie die übrigen `handle`-Blöcke ein. Darin hält eine verschachtelte `route` die Reihenfolge fest: Nur bei `/map.html` wird der URI-Pfad zuerst per `uri replace /map.html /map` umgeschrieben und anschließend der vollständige `{uri}` mit HTTP 308 als Redirectziel verwendet. Dadurch bleibt die bestehende Query unverändert Bestandteil des URI, während die alte Pfadkomponente verschwindet. Der zuvor verwendete optionale Query-Platzhalter `{?query}` ist hier nicht der Produktionsvertrag, weil der Live-Readback gezeigt hat, dass die konkrete Redirectform die Query verlor. Eine Query wie `next=%2Fmap.html` darf durch die Pfadumschreibung nicht verändert werden.

Die `handle`-Grenze ist Teil des Produktionsvertrags: Caddys Adapter sortiert top-level `handle` vor top-level `route`. Eine eigenständige `route @legacyMapHtml` kann deshalb trotz früherer Textposition hinter den catch-all `handle` mit `file_server` geraten. Genau dieser Zustand wurde nach dem Deploy von `ac79f4a1332f6797b98791f9abf2951d88b37893` live beobachtet: Die aktive Containerkonfiguration enthielt die Redirectregel, die adaptierte Handlerkette platzierte jedoch den catch-all statischen Handler davor und `/map.html` blieb HTTP 200. Die Redirectregel muss daher als spezifischer `handle` vor dem statischen `try_files`- beziehungsweise SPA-Fallback adaptiert werden; nur die innere `route` dient der unveränderten Reihenfolge `uri` → `redir`.
