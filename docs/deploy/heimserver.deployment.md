...
## Standard-Kommandos
- Stack:
  - `docker compose --env-file /opt/weltgewebe/.env -p weltgewebe -f infra/compose/compose.prod.yml -f infra/compose/compose.prod.override.yml up -d --build`
- Edge:
...
## Login (Magic Link)
- Request:
  - `curl -kfsS -X POST https://weltgewebe.home.arpa/api/auth/login/request -H 'content-type: application/json' -d '{"email":"..."}'`
