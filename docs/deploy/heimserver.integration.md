...
* **`auto` (Default):** Nutzt eine Heuristik. Explizites `BUILD_WEB=no` deaktiviert die
  ...
  dass Build-Artefakte erzeugt werden müssen. Build-Artefakte zu erzeugen heißt nicht automatisch, dass sie schon operativ ausgeliefert werden. Das Script garantiert nicht die operative Auslieferung durch
