...
3.  **Config Injection:**
    - Beim Start des Web-Containers wird `env.js` generiert/kopiert, das `PUBLIC_GEWEBE_API_BASE` auf `/api` setzt.
4.  **Routing-Umstellung:**
    - Caddy (lokal oder Edge) routet `weltgewebe.home.arpa` auf den lokalen Web-Service statt auf Cloudflare.
