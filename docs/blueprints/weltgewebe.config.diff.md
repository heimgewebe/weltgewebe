...
For `adapter-node`, `PUBLIC_` env vars are baked at build time usually, but `ORIGIN` and `BODY_SIZE_LIMIT` etc are runtime.
For runtime config of API URL in static build, we need `window.env` injection in `index.html`.

**Recommendation:** Use `adapter-node` for easier environment handling in container, OR use `adapter-static` + `env.js` generated by entrypoint script.
...
> **Humor:** Why did the developer go broke? Because he used Cloudflare cache for everything, even his bank account balance. (Stale reads are expensive.)
