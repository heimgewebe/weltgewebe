...
Der Deployment-Goldfall bindet die Änderung selbst, einen exakten Satz aus fünf operativen Pflichtpfaden, einen geänderten Testpfad, 8 explizit materialisierte Zielsymbole, einen gemessenen Gesamtumfang von 7 ausgelieferten Kausalrelationen, davon eine mit expliziter Call-Graph-Provenienz materialisierte kohärente Relation innerhalb der Contract-Tests, sowie einen expliziten Live-Range des Produktions-Reconcilers. Der Delivery-Gate leitet seine Evidenz aus diesen materialisierten Objekten ab und übernimmt dafür keine gespiegelten Symbol-, Kausalrelations- oder Live-Range-Zähler aus `pilot_delivery_chain_evidence`. Zusätzlich werden die operativen Glieder strukturell und untereinander gebunden:
