...
Für diesen Livefall gilt `delivery_completeness_required=false`: Der Revisionsdiff kennt 95 Direktänderungen, die Capsule berücksichtigt wegen des deterministischen Lane-Caps 16 davon und liefert diese 16 aus; 79 werden als `policy_omitted` ausgewiesen. Der Validator verlangt für `controlled_live` ausdrücklich `included_direct < available_direct` und mindestens eine reale Direktpfad-Auslassung. Ein konsistent umgeschriebener Vollständigkeitsfall kann daher nicht mehr als kontrollierter Budget-/Policy-Livefall bestehen. Entsprechend wird keine `critical_path_coverage=1.0` behauptet. Das Modell trennt damit die vollständige Revisionsinventarisierung von der tatsächlich ausgelieferten, budgetbegrenzten Capsule.
...
Policy-Begrenzung und Budgetdegradierung sind getrennt modelliert. Acht promotionsrelevante Budget-Lanes werden mit `available`, `considered`, `included`, `policy_omitted` und `budget_omitted` gezählt: `direct_changes`, `related_tests`, `target_symbols`, `causal_relations`, `live_ranges`, `citations`, `entry_manifest` und `query_snippets`. DB/Auth und Web/Karte sind nicht policy-begrenzt und erschöpfen nur `query_snippets`; ihre `entry_manifest`-Lane wird vollständig ausgeliefert. Beim Deployment begrenzt die Sampling-Policy `target_symbols` und `causal_relations`; zusätzlich erschöpft das Byte-Budget `causal_relations`, `live_ranges`, `citations`, `entry_manifest` und `query_snippets`. Im Livefall begrenzt die Policy `direct_changes`, `related_tests` und `target_symbols`; das Byte-Budget erschöpft anschließend `live_ranges`, `citations`, `entry_manifest` und `query_snippets`. Die deklarierte `exhausted_lanes`-Menge muss exakt den Lanes mit `budget_omitted > 0` entsprechen; damit können `entry_manifest` oder `query_snippets` nicht mehr unbelegt ergänzt oder entfernt werden. Für jede beobachtete Lane gilt rechnerisch `policy_omitted = available - considered` und `budget_omitted = considered - included`. Ob erforderliche Evidenz fehlt, wird nicht durch ein gespeichertes `required_lane_loss`-Urteil behauptet, sondern aus den konkreten Goldfall-, Direktänderungs-, Related-Test- und Lane-Regeln berechnet. `impact_context_blocked` tritt in keinem Fall auf.
...
Der Deployment-Goldfall bindet die Änderung selbst, einen exakten Satz aus fünf operativen Pflichtpfaden, einen geänderten Testpfad, 8 explizit materialisierte Zielsymbole, einen gemessenen Gesamtumfang von 7 ausgelieferten Kausalrelationen, davon eine mit expliziter Call-Graph-Provenienz materialisierte kohärente Relation innerhalb der Contract-Tests, sowie einen expliziten Live-Range des Produktions-Reconcilers. Materialisierte Symbolidentitäten, Symbol-Pfad-/Range-Bindungen und repräsentative Kausalrelationsidentitäten müssen eindeutig sein; ebenso sind Fall-IDs, Revisions-Direktpfade, Goldfall-Critical-Paths und Related-Test-Pfade duplikatfrei. Die Retrieval-Lanes bilden je Fall eine eindeutige, disjunkte Partition der bekannten Lane-Menge; unbekannte oder doppelte Lane-Namen blockieren. Rollback-/Recovery-Risiken benötigen eindeutige Risikonamen, sodass eine Mindestanzahl nicht durch Duplikate erfüllt werden kann. Contract- und Recovery-Evidenz binden denselben exakten Contract-Testpfad `scripts/ci/tests/test_production_reconciler_contract.py`. Der Delivery-Gate leitet seine Evidenz aus diesen materialisierten Objekten ab und übernimmt dafür keine gespiegelten Symbol-, Kausalrelations- oder Live-Range-Zähler aus `pilot_delivery_chain_evidence`. Zusätzlich werden die operativen Glieder strukturell und untereinander gebunden:
...
- **Recovery:** `docs/deploy/merge-to-live.md#L162-L171` bindet direkten Recovery-Aufruf, gemeinsame `flock`-Sperre, wirkungslose Konkurrenzablehnung und `EX_TEMPFAIL 75`. Die Zeilenspanne wird gegen den historischen Deployment-Zielcommit `7a1b5943524affd90b79cd8769f5e48b3f1d4b22` neu gelesen und muss den gebundenen SHA-256 `460ea84cd60b2907f7264706d439f3aabee56a0dbc368f09b711214cb33e6535` ergeben.
