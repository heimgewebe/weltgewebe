...
- [~] Agent Safety Control Layer · Blueprint gemergt; Safety-Preflight, Readiness Hard Fail, Claim-Spine, Agent-Contracts, Non-Ideal-Guard und Handoff-Validierung sind ueber `AGENT-SAFE-001` bis `AGENT-SAFE-005` belegt; `AGENT-SAFE-006` fuehrt den read-only Dry-Run-Runner ein. Run-Evidence und Write Mode bleiben offen · [blueprint-agent-safety-control-layer.md](blueprints/blueprint-agent-safety-control-layer.md)
