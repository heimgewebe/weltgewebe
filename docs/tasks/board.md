...
| OPT-ARC-001 | api | JSONL → PostgreSQL | partial | high | `docs/blueprints/domain-data-postgres-cutover.md`, `apps/api/migrations/20260531000001_create_domain_nodes.up.sql`, `apps/api/migrations/20260531000002_create_domain_edges.up.sql`, `apps/api/migrations/20260531000003_create_domain_accounts.up.sql`, `apps/api/tests/db_domain_schema_migrations.rs`, `apps/api/tests/db_domain_backfill.rs`, `docs/reports/domain-backfill-proof.md`, `.github/workflows/api.yml` (`db-domain-schema-migrations-proof`, `db-domain-backfill-proof`) | Phase B + Phase C (Backfill-Proof-Code) implementiert; CLI-Validierung grün; PR-CI-Laufbeleg ausstehend (db-domain-schema-migrations-proof + db-domain-backfill-proof); nächste Aktion: Phase D (Read-Path-Switch hinter explizitem Config-Gate) |
...
|---|---|---|---|
| OPT-ARC-001 | PR-CI-Laufbeleg für `db-domain-schema-migrations-proof` und `db-domain-backfill-proof` ausstehend; kein Read-/Write-Path-Cutover | Grünen CI-Lauf beider Jobs belegen; dann Phase D (Read-Path-Switch) angehen | JSONL bleibt aktive Datenquelle bis Phase D/E |
