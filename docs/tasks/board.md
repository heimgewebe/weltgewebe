...
| DB-PROOF-001 | api | Edge-Orphan- und Referenz-Audit | partial | high | `apps/api/migrations/20260531000002_create_domain_edges.up.sql`, `scripts/docmeta/audit_domain_edge_references.py`, `scripts/docmeta/tests/test_audit_domain_edge_references.py`, `docs/reports/domain-edge-reference-audit.md` | Audit-Harness, Tests und Report vorhanden; PostgreSQL-Smoke lief gegen leere DB und belegt keine repräsentative FK-Readiness; repräsentativer Runtime-/Deployment-Datenlauf mit auditable_edges_total > 0 fehlt; FK-vs-Guard-Policy bleibt offen; keine Migration. |
| DOMAIN-PG-002 | infra | Single-Instance-Invariante oder Multi-Instance-Kohärenz entscheiden | done | high | `docs/reports/domain-postgres-instance-coherence-decision.md`, `docs/blueprints/domain-data-postgres-cutover.md` (Abschnitt „Instance Coherence Boundary"), `scripts/guard/domain-single-instance-guard.sh`, `scripts/tests/test_domain_single_instance_guard.sh` | Option A (Single-Instance-Invariant) entschieden und im Cutover-Blueprint verankert; prozesslokale Domain-/Auth-Caches inventarisiert; statischer Guard blockiert offensichtliche Scale-out-Drift fail-closed (api-Replicas != 0/1, `--scale api` != 0/1, API-Upstream zusammen mit einem weiteren Upstream auf derselben Caddy reverse_proxy/to-Direktivzeile) und ist getestet; kein Cross-Instance-Kohärenz-Claim; kein PostgreSQL-Cutover |
