...
| AGENT-SAFE-007 | governance | Run Evidence Lite für erfolgreiche Agent-Dry-Runs einführen | partial | high | `contracts/agent/validation.schema.json`, `contracts/agent/run-result.schema.json`, `scripts/agent/json_contract.py`, `scripts/agent/run_task.py`, `scripts/agent/validate_agent_contracts.py`, `scripts/agent/tests/test_json_contract.py`, `scripts/agent/tests/test_run_task.py`, `scripts/contracts-agent-check.sh`, `scripts/docmeta/generate_agent_readiness.py`, `scripts/docmeta/tests/test_generate_agent_readiness.py`, `tests/fixtures/agent/validation-valid.json`, `tests/fixtures/agent/run-result-valid.json`, `.github/workflows/agent-safety-preflight.yml`, `docs/reference/agent-run-evidence-lite.md`, PR #1265 | Implementierung und lokale Beweise sind vorhanden; Merge und post-merge Verifikation stehen aus. Git-Identität, Output-Pfadidentität, geordnete Contracts und Fehler nach Publikation sind fail-closed abgesichert; Failure-Evidence, externe Attestierung, Patch-Anwendung und Write Mode bleiben Folgearbeit |
