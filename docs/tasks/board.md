...
| OPT-ARC-001 | api | JSONL → PostgreSQL | partial | high | `docs/blueprints/domain-data-postgres-cutover.md`, `apps/api/migrations/20260531000001_create_domain_nodes.up.sql`, `apps/api/migrations/20260531000002_create_domain_edges.up.sql`, `apps/api/migrations/20260531000003_create_domain_accounts.up.sql`, `apps/api/tests/db_domain_schema_migrations.rs`, `apps/api/tests/db_domain_backfill.rs`, `docs/reports/domain-backfill-proof.md`, `apps/api/tests/db_domain_read_path.rs`, `docs/reports/domain-read-path-proof.md`, `.github/workflows/api.yml` (`db-domain-schema-migrations-proof`, `db-domain-backfill-proof`) | Phase B + Phase C (Backfill-Proof-Code) implementiert; Phase D (Read-Path-Proof) implementiert: `db_domain_read_path`-Suite grün kompiliert, PR-CI-Laufbeleg mit `--include-ignored` und PostgreSQL-Service ausstehend; nächste Aktion: Phase E (Write-Path-Switch) |
...
|---|---|---|---|
| OPT-ARC-001 | PR-CI-Laufbeleg für `db-domain-schema-migrations-proof`, `db-domain-backfill-proof` und `db_domain_read_path` ausstehend; kein Write-Path-Cutover | Grünen CI-Lauf aller DB-Proof-Jobs belegen; dann Phase E (Write-Path-Switch) angehen | JSONL bleibt aktive Datenquelle bis Phase E |
