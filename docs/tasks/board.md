...
| OPT-ARC-001 | api | JSONL → PostgreSQL | partial | high | `docs/blueprints/domain-data-postgres-cutover.md`, `apps/api/migrations/20260531000001_create_domain_nodes.up.sql`, `apps/api/migrations/20260531000002_create_domain_edges.up.sql`, `apps/api/migrations/20260531000003_create_domain_accounts.up.sql`, `apps/api/tests/db_domain_schema_migrations.rs`, `apps/api/tests/db_domain_backfill.rs`, `apps/api/tests/db_domain_read_path.rs`, `docs/reports/domain-backfill-proof.md`, `docs/reports/domain-read-path-proof.md`, `.github/workflows/api.yml` (`db-domain-schema-migrations-proof`, `db-domain-backfill-proof`, `db-domain-read-path-proof`) | Phase B + Phase C implementiert; Phase D opt-in/read-only implementiert; db_domain_read_path-Suite als lokaler PostgreSQL-Proof vorbereitet; lokaler Loader-Proof berichtet; PR-CI-Beleg ausstehend; nächste Aktion: PR-CI-Beleg abwarten, Phase E bleibt offen |
