...
| OPT-ARC-001 | api | JSONL → PostgreSQL | partial | high | `docs/blueprints/domain-data-postgres-cutover.md`, `apps/api/migrations/20260531000001_create_domain_nodes.up.sql`, `apps/api/migrations/20260531000002_create_domain_edges.up.sql`, `apps/api/migrations/20260531000003_create_domain_accounts.up.sql`, `apps/api/tests/db_domain_schema_migrations.rs`, `apps/api/tests/db_domain_backfill.rs`, `apps/api/tests/db_domain_read_path.rs`, `apps/api/src/domain_db.rs`, `apps/api/src/config.rs` (`DomainReadSource`, `WELTGEWEBE_DOMAIN_READ_SOURCE`), `docs/reports/domain-backfill-proof.md`, `docs/reports/domain-read-path-proof.md`, `.github/workflows/api.yml` (`db-domain-schema-migrations-proof`, `db-domain-backfill-proof`, `db-domain-read-path-proof`) | Phase B + Phase C (Backfill-Proof-Code) + Phase D (Read-Path-Switch hinter `WELTGEWEBE_DOMAIN_READ_SOURCE=postgres`) implementiert; Loader befüllen dieselben Cache-Typen wie JSONL, Account-Privacy-Rekonstruktion geteilt via `map_json_to_public_account`; PR-CI-Laufbelege ausstehend (db-domain-schema-migrations-proof + db-domain-backfill-proof + db-domain-read-path-proof); nächste Aktion: Phase E (Write-Path-Switch) hinter eigenem Config-Gate |
...
|---|---|---|---|
| OPT-ARC-001 | PR-CI-Laufbelege für `db-domain-schema-migrations-proof`, `db-domain-backfill-proof` und `db-domain-read-path-proof` ausstehend; kein Write-Path-Cutover | Grünen CI-Lauf aller drei Jobs belegen; dann Phase E (Write-Path-Switch) hinter eigenem Config-Gate angehen | JSONL bleibt Default und Write-Wahrheit bis Phase E; Phase D ist Code-Stand und wartet auf grüne CI-Proof-Runs |
