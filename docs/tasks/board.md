...
| AGENT-SAFE-005 | governance | Deterministische Agent-Handoff-Validierung einführen | partial | high | `contracts/agent/handoff.schema.json`, `scripts/agent/json_contract.py`, `scripts/agent/validate_handoff.py`, `scripts/contracts-agent-check.sh`, `scripts/agent/tests/test_validate_handoff.py`, `tests/fixtures/agent/handoff-*.json`, `docs/reference/agent-handoff-contract.md`, `.github/workflows/agent-safety-preflight.yml`, `.github/workflows/contracts-validate.yml` | Draft-PR #1261: Review-Handoff wird gegen den kanonischen Task-Contract geprüft; Main-Integration steht aus, Run-Attestierung und Runner bleiben bewusst spätere Slices |
