...
| AGENT-SAFE-007 | governance | Run Evidence Lite für erfolgreiche Agent-Dry-Runs einführen | partial | high | `contracts/agent/validation.schema.json`, `contracts/agent/run-result.schema.json`, `scripts/agent/run_task.py`, `scripts/agent/tests/test_run_task.py`, `scripts/docmeta/generate_agent_readiness.py`, `.github/workflows/agent-safety-preflight.yml`, `docs/reference/agent-run-evidence-lite.md`, PR #1265 | Implementierung und lokale Beweise sind vorhanden; Merge und post-merge Verifikation stehen aus. Blockierte und fehlerhafte Runs, externe Attestierung, Patch-Anwendung und Write Mode bleiben Folgearbeit |
