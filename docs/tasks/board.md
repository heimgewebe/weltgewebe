...
|---|---|---|---|
| OPT-ARC-001 | PR-CI-Laufbelege für `db-domain-account-write-path-proof`, `db-domain-node-write-path-proof` und `db-domain-edge-write-path-proof` ausstehend; Step-up-E-Mail-Persistenz und WebAuthn-User-ID-Writeback offen; Runtime-Smoke und Cutover ausstehend | Grünen PR-CI-Lauf der drei Write-Path-Jobs belegen, danach offene Migrationsteile bzw. Cutover-Proof vorbereiten | JSONL bleibt Default-Lesequelle und Write-Truth bis vollständiger Cutover; POST /accounts, PATCH /nodes und POST /edges schreiben optional PostgreSQL; Schema-, Backfill- und Read-Path-Proof sind in PR-CI grün (PR #1165, pull_request-Run 27041187630) |
