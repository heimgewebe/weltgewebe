...
3. **Build Diagnosis**: `/_app/version.json` acts as the technical diagnostic source for the currently deployed frontend build. It is strictly served with `no-store` to ensure the browser always retrieves the current server state without caching interference.
