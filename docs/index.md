---
id: docs.index
title: Dokumentations-Index
doc_type: index
status: active
canonicality: canonical
summary: >
  Kanonischer Einstiegspunkt für die Dokumentation im Weltgewebe-Repository.
---

Willkommen im kanonischen Dokumentationsindex für das Weltgewebe-Projekt. Diese Seite bietet den Einstieg
in die Struktur und die wichtigsten Dokumente.

## Einstieg

- [Vision & Leitidee](vision.md)
- [Systematik & Strukturüberblick](overview/zusammenstellung.md)
- [Einführung in den Inhalt](overview/inhalt.md)
- [Technologie Stack](techstack.md)

## Architektur und Struktur

- [Architekturstruktur](architekturstruktur.md)
- [Datenmodell](datenmodell.md)
- [Deployment](deployment.md)

## Repo Observatorium

Dieses Repository analysiert sich selbst. Die folgenden Berichte werden generiert:

- [System Map](_generated/system-map.md)
- [Doc Index](_generated/doc-index.md)
- [Architecture Drift](_generated/architecture-drift.md)
- [Doc Coverage](_generated/doc-coverage.md)
- [Knowledge Gaps](_generated/knowledge-gaps.md)
- [Implicit Dependencies](_generated/implicit-dependencies.md)
- [Change Resonance](_generated/change-resonance.md)
- [Staleness Report](_generated/staleness-report.md)
- [Agent Readiness](_generated/agent-readiness.md)
- [Orphans](_generated/orphans.md)
