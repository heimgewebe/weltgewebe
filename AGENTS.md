...
•Immer echte Dateien bevorzugen:
Bevor du Code vorschlägst oder analysierst, musst du nach der realen Datei im Repo suchen und von dort aus arbeiten. Rate nicht frei, wenn die Datei existiert.
•Keine „stilisierten“ Snippets:
Verwende keine verkürzten Schreibweisen wie SECONDS end oder zerstückelte Redirections (devtcpHOSTPORT). Alles, was du zeigst, muss so in einer echten Datei kompilierbar bzw. ausführbar sein.
•Vollständige Blöcke:
Wenn du Funktionen/Skripte änderst, zeige immer den ganzen betroffenen Block (z.B. komplette Funktion, komplettes Skript), nicht nur einzelne zerhackte Zeilen.
•Kennzeichnung von Auslassungen:
Wenn du Teile weglässt, markiere das explizit mit Kommentaren wie // ... oder # ..., ohne dabei Syntax zu zerstören.
...
•Keine überflüssigen Zeichen am Ende von Template Strings (z.B. kein abschließendes Komma nach ${actual}).
•Fehlermeldungen müssen klar, einzeilig und ohne überflüssige Interpunktion sein.
•Strikte Typprüfungen:
•Wenn du Performance-Budgets oder Konfigwerte prüfst, nutze Muster wie:
...
•POSIX-/Bash-Syntax niemals „optisch vereinfachen“:
...
•Verwende klare Standardwerte mit ${VAR:-default} und weise sie oben zu, z.B.:
...
•Zeit-Logik korrekt schreiben:
•Integer-Deklaration sauber:
...
•Kein pseudo-Code in echten Skripten:
Wenn du ein Snippet als Beispiel zeigst, das nicht 1:1 lauffähig ist, musst du das ausdrücklich als Pseudocode markieren. In echten Dateien darfst du nur ausführbaren Code vorschlagen.

⸻

4. Verhalten bei Unsicherheit
•Wenn du dir bei einer Datei, Syntax oder Semantik nicht sicher bist:
•sage das explizit,
•schlage eine mögliche Variante vor,
...
5. Zielbild
•Code-Vorschläge aus dieser Umgebung sollen:
•direkt lauffähig,
•syntaktisch korrekt,
•CI-tauglich
sein – ohne händische Nachkorrekturen von offensichtlichen Tipp- und Syntaxfehlern.
