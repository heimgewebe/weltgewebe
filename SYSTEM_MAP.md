# SYSTEM_MAP

## Zone: norm

| ID             | File                                                             | Status    | Last Reviewed | Depends On |
|----------------|------------------------------------------------------------------|-----------|---------------|------------|
| docmeta.schema | [architecture/docmeta.schema.md](architecture/docmeta.schema.md) | canonical | 2026-02-28    |            |
| overview       | [architecture/overview.md](architecture/overview.md)             | canonical | 2026-02-28    |            |
| security       | [architecture/security.md](architecture/security.md)             | canonical | 2026-02-28    |            |

## Zone: reality

| ID             | File                                   | Status    | Last Reviewed | Depends On |
|----------------|----------------------------------------|-----------|---------------|------------|
| runtime.readme | [runtime/README.md](runtime/README.md) | canonical | 2026-02-28    |            |

## Zone: runbooks

| ID              | File                                     | Status    | Last Reviewed | Depends On |
|-----------------|------------------------------------------|-----------|---------------|------------|
| runbooks.readme | [runbooks/README.md](runbooks/README.md) | canonical | 2026-02-28    |            |

## Automated Checks

- [scripts/docmeta/check_doc_review_age.py](scripts/docmeta/check_doc_review_age.py)
- [scripts/docmeta/check_repo_index_consistency.py](scripts/docmeta/check_repo_index_consistency.py)
- [scripts/docmeta/generate_system_map.py](scripts/docmeta/generate_system_map.py)
