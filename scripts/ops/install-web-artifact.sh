...
if [[ -n "$artifact_commit" ]]; then [[ "$artifact_commit" == "$WEB_ARTIFACT_COMMIT" ]] || fail "artifact commit mismatch"
...
if [[ -L "$WEB_ROOT" ]]; then
  previous_kind=symlink; previous_target="$(readlink "$WEB_ROOT")"
...
rollback() {
  ...
  case "$previous_kind" in
    symlink) ln -s "$previous_target" "$tmp_link"; mv -Tf "$tmp_link" "$WEB_ROOT" ;;
...
switched=1
if ! run_cmd "$CADDY_RELOAD_CMD"; then rollback; fail "Caddy recreate command failed; previous web release restored"; fi

readback="$(mktemp)"; headers="$(mktemp)"
if ! curl -fsS -D "$headers" -o "$readback" "$PUBLIC_VERSION_URL"; then rollback; fail "public version readback failed; previous web release restored"; fi
if ! grep -iq '^Cache-Control:.*no-store' "$headers"; then rollback; fail "public version readback lacks no-store; previous web release restored"; fi
public_build="$(awk 'BEGIN{IGNORECASE=1} /^X-Weltgewebe-Build:/ {sub(/^[^:]+:[[:space:]]*/,""); sub(/\r$/,""); print; exit}' "$headers")"
public_version="$(read_version_field "$readback" 1)"; public_commit="$(read_version_field "$readback" 3)"
rm -f "$readback" "$headers"
[[ "$public_version" == "$WEB_ARTIFACT_VERSION" ]] || { rollback; fail "public version mismatch; previous web release restored"; }
[[ -z "$public_commit" || "$public_commit" == "$WEB_ARTIFACT_COMMIT" ]] || { rollback; fail "public commit mismatch; previous web release restored"; }
[[ "$public_build" == "$WEB_ARTIFACT_VERSION" || "$public_build" == "$WEB_ARTIFACT_COMMIT" ]] || { rollback; fail "public build header mismatch; previous web release restored"; }
...
if (( WEB_RELEASE_RETENTION > 0 )); then
  ...
  for dir in "${old[@]}"; do
    ...
    kept=$((kept+1)); (( kept <= WEB_RELEASE_RETENTION )) || rm -rf "$dir"
  done
fi
