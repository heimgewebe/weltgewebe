...
VALID_POLICY_TOP_LEVEL_KEYS = {"kind", "version", "families"}
VALID_POLICY_FAMILY_KEYS = {
    "id",
    "claim_id_prefix",
    "entry_id_prefix",
    "registry_doc",
...
STRING_REQUIRED_ACTIVE_FAMILY_FIELDS = (
    "id",
    "claim_id_prefix",
    "entry_id_prefix",
    "registry_doc",
...
def _set_policy_key(
    ...
    *,
    ...
    """Assign a policy key, rejecting duplicates with a located error.

    Centralizes duplicate-key detection for both the top level and family
    scopes so a repeated key — including an inline ``- id:`` followed by a
    later ``id:`` field — fails loudly with its scope, key name, and line
    number instead of being silently overwritten.
    """
    if key in target:
        raise ValueError(
            f"Duplicate {scope} policy key {key!r} at line {line_no}"
        )
    target[key] = value
...
def _parse_freshness_scope_policy(text: str) -> dict[str, object]:
    ...
    for line_no, raw_line in enumerate(text.splitlines(), start=1):
        ...
        if indent == 0:
            ...
            key = key.strip()
            value = value.strip()
            if key not in VALID_POLICY_TOP_LEVEL_KEYS:
                raise ValueError(
                    f"Unknown top-level policy key {key!r} at line {line_no}"
                )
            if key == "families":
                if value:
                ...
                _set_policy_key(
                    data, "families", new_families, scope="top-level", line_no=line_no
                )
        ...
        if stripped == "-" or stripped.startswith("- "):
            ...
            if item:
                ...
                fkey = key.strip()
                if fkey not in VALID_POLICY_FAMILY_KEYS:
                    raise ValueError(
                        f"Unknown policy family key {fkey!r} at line {line_no}"
                    )
                _set_policy_key(
                    current,
                    fkey,
...
def _prefixes_overlap(a: str, b: str) -> bool:
    """True if either string is a prefix of the other (covers both exact and subprefix overlap)."""
    return a.startswith(b) or b.startswith(a)


def _validate_scope_policy(
    data: object,
    expected_registry_doc: str,
    ...
    for raw in raw_families:
        ...
        _raw_rlc = raw.get("require_live_check")
        if _raw_rlc is not None and not isinstance(_raw_rlc, bool):
            return invalid(
                f"Policy family '{fam_id}' field 'require_live_check' must be a boolean"
            )
        require_live_check = _raw_rlc if _raw_rlc is not None else False

        # Inactive families are syntactically validated above but ignored for scope.
        if status != "active":
            continue

        if "require_live_check" not in raw:
            return invalid(
                f"Active policy family '{fam_id}' must explicitly set 'require_live_check'"
            )

        for field in STRING_REQUIRED_ACTIVE_FAMILY_FIELDS:
            value = raw.get(field)
        ...
        if raw["registry_doc"] != expected_registry_doc:
            return invalid(
                f"Active policy family '{fam_id}' registry_doc must be "
                f"'{expected_registry_doc}', got: {raw['registry_doc']!r}"
            )

        claim_prefix = raw["claim_id_prefix"]
        entry_prefix = raw["entry_id_prefix"]
        for existing in seen_claim_prefixes:
            if _prefixes_overlap(claim_prefix, existing):
                return invalid(
                    f"Overlapping claim_id_prefix {claim_prefix!r} with existing {existing!r}"
                )
        for existing in seen_entry_prefixes:
            if _prefixes_overlap(entry_prefix, existing):
                return invalid(
                    f"Overlapping entry_id_prefix {entry_prefix!r} with existing {existing!r}"
                )
...
def load_scope_policy(
    ...
    return _validate_scope_policy(data, expected_registry_doc)
...
def validate_registry_data(
    ...
    for entry in entries:
        ...
        doc = entry.get("doc")
        if family is not None and doc != family.registry_doc:
            findings.append(
                _finding(
                    "DOC_MISMATCH",
                    entry_id,
                    f"'doc' must be '{family.registry_doc}', got: {doc!r}",
                )
            )
        ...
        # requirement structural and never reads the wall clock. An empty
        # evidence list must not bypass the check: any([]) is False, so an
        # empty list still raises the live-check finding (alongside, but never
        # replaced by, EVIDENCE_EMPTY).
        if (
            family is not None
...
def run_validation(
    ...
    families, policy_findings = load_scope_policy(policy_path, claims)
    if families is None:
